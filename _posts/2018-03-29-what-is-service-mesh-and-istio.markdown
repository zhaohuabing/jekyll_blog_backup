...
这里我们可以类比SDN的概念，控制面就类似于SDN网管中的控制器，负责路由策略的指定和路由规则下发；


类似于SDN网络中交换机，负责数据包的转发。由于微服务的所有通讯都有服务网格基础设施层提供，通过控制面板和数据面板的配合，可以对这些通讯进行监控、托管和控制，以实现微服务灰度发布，调用分布式追踪，故障注入模拟测试，动态路由规则，微服务闭环控制等管控功能。
...
Mixer是如何能够做到这一点的呢？看一下Mixer的架构:
![](\img\in-post\2018-03-29-what-is-service-mesh-and-istio\mixer.png)

首先，Sidecar会从每一次请求中收集相关信息，如请求的路径，时间，源IP，目地服务，tracing头等，并请这些属性上报给Mixer。Mixer和后端服务之间是通过适配器进行连接的，Mixer将Sidecar上报的内容通过适配器发送给后端服务。
...
从上图可以看到，为了实现策略控制和遥测数据收集，Istio在Envoy中增加了一个Mixer Filter。该Mixer Filter和后端的Mixer组件进行通讯，完成策略控制和遥测数据收集等功能。两个微服务之间的每一次请求调用都会经过两次Mixer Filter的处理，一次在发送端，一次在接收端。在Envoy的Mixer Filter中保留有缓存，遥测数据收集后会通过批量上报的方式进行处理，保证了处理的效率，避免了Mixer对服务调用引入较大延时。
